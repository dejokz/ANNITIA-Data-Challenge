...
class TrajectoryFeatureEngineer:
    ...
    def calculate_clinical_scores(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for visit in range(1, max_visits + 1):
            ...
            if all(col in result.columns for col in required_cols):
                ...
                # Using ULN_AST = 40 as standard upper limit
                ULN_AST = 40
                ...
                # AST/ALT ratio (important for cirrhosis detection)
                result[f'ast_alt_ratio_v{visit}'] = (
                    result[ast_col] / (result[alt_col] + 0.001)
                )
        
        # Add calculated scores to processing list
        self.visit_vars.extend(['fib4', 'apri', 'ast_alt_ratio'])
        
        logger.info(f"Clinical scores calculated. New variables: fib4, apri, ast_alt_ratio")
        return result
    ...
    def extract_trajectory_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        These capture disease dynamics over time.
        """
        ...
        for var in tqdm(self.visit_vars, desc="Processing variables"):
            ...
            # === CLINICAL THRESHOLDS ===
            if var == 'fib4':
                # Time spent in high-risk zone (FIB-4 > 2.67)
                ...
                # Visit at which max occurred (early vs late progression)
            ...
            elif var == 'fibs_stiffness_med_BM_1':  # LSM
    ...
    def extract_cross_nit_concordance(self, df: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        Discordance can indicate different pathophysiology or measurement error.
        """
        logger.info("Extracting cross-NIT concordance features...")
        
        # Get trajectory features for key NITs
        nit_pairs = [
        ...
        for nit1, nit2 in nit_pairs:
            ...
            if slope1_col in features.columns and slope2_col in features.columns:
                # Do both tests agree on direction of change?
                ...
                # Do both agree on high-risk status?
        ...
        # Composite trajectory: Are ALL key NITs worsening?
...
class MedicalCorroboration:
    ...
    This prevents overfitting to spurious correlations.
    """
    ...
    def validate_feature(self, feature_name: str, correlation: float, 
        ...
        """
        Validate a single feature against medical expectations.
        Returns True if feature passes validation, False otherwise.
        """
        is_valid = True
        messages = []
    ...
    def corroborate_features(self, X: pd.DataFrame, y_event: np.ndarray) -> List[str]:
        """
        Run medical corroboration on all features.
        Returns list of features that passed validation.
        """
        logger.info("\n" + "="*80)
        ...
        # Define medical expectations
        medical_expectations = {
            # Fibrosis markers should correlate POSITIVELY with events
            ...
            # LSM (direct stiffness measurement)
            ...
            # Platelets (inverse relationship - low platelets = bad)
...
class SurvivalModelEnsemble:
    ...
    def prepare_survival_target(self, df: pd.DataFrame, outcome: str = 'hepatic') -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        # Calculate last observed age from Age_v columns
    ...
    def create_preprocessing_pipeline(self, max_missing_rate: float = 0.60):
        """Create preprocessing pipeline with missing value filtering."""
        return Pipeline([
    ...
    def fit_fold(self, X_train: pd.DataFrame, y_train: np.ndarray, 
        ...
        """Fit a single model on one fold."""
        
        if model_type == 'rsf':
            model = RandomSurvivalForest(
                n_estimators=500,
                min_samples_leaf=30,  # Larger leaves for regularization
    ...
    def cross_validate(self, X: pd.DataFrame, y: np.ndarray, 
        ...
        """
        Stratified cross-validation for survival analysis.
        """
